...
"""
예수금 없는 원래 버전의 완전한 데이터베이스 설정 스크립트 재생성
"""

import random
from datetime import datetime, timedelta

def restore_original_complete_setup():
    """예수금 없는 원래 버전으로 완전한 데이터베이스 설정 스크립트 생성"""
    
    print("🔄 예수금 제외한 원본 완전한 설정 스크립트 생성 중...")
    
    sql_content = """-- 원본 완전한 데이터베이스 정리 및 새 데이터 입력 스크립트 (예수금 제외)
-- 생성일시: """ + datetime.now().strftime('%Y-%m-%d %H:%M:%S') + """

USE kpsdb;

-- ========================================
-- 1단계: 기존 데이터 완전 삭제 및 스키마 복원
-- ========================================
...
-- 예수금 컬럼이 있다면 제거 (없다면 무시됨)
ALTER TABLE customer_balance DROP COLUMN IF EXISTS deposit_amount;
...
"""
    ...
    # 2. 고객잔고 (예수금 없는 원본 버전)
    sql_content += """-- 2. 고객잔고 (20개 종목 - 원본 스키마)
...
-- 완료 메시지
SELECT '🎉 원본 스키마로 모든 데이터 입력이 성공적으로 완료되었습니다!' as result;

"""
    
    # 파일 저장
    with open('complete_database_setup_original.sql', 'w', encoding='utf-8') as f:
        f.write(sql_content)
    
    print("✅ 원본 완전한 데이터베이스 설정 스크립트 생성 완료!")
    print("📁 파일: complete_database_setup_original.sql")
    print("🔧 포함 기능:")
    print("   - 기존 데이터 완전 삭제")
    print("   - 예수금 컬럼 제거 (IF EXISTS)")
    print("   - 2,500개 순차 종목코드")
    print("   - 20개 고객잔고 (원본 스키마)")
    print("   - 930건 매매내역")
    print("   - 원본 테이블 구조 확인")
    print("♻️  예수금 없는 원본 상태로 복원")

if __name__ == "__main__":
    restore_original_complete_setup()